...
int main(int argc, char **argv)
{
  ...
  private_nh.param("file_directory", file_directory, std::string("/home/gjm/AAA/")); // 此处需要修改为自己pcd文件的路径
  ROS_INFO("*** file_directory = %s ***\n", file_directory.c_str());

  private_nh.param("file_name", file_name, std::string("file_c")); // 此处"pcd_name"需要修改为自己的pcd文件名，无需.pcd
  ...
  // 用于修改pcd的参数文件
  /* thre_z_min: 设置一个名为 thre_z_min 的参数，用于指定 Z 轴的最小阈值 (默认为 0.5)。
  thre_z_max: 设置一个名为 thre_z_max 的参数，用于指定 Z 轴的最大阈值 (默认为 2.0)。
  flag_pass_through: 设置一个名为 flag_pass_through 的参数，用于控制某种行为 (默认为 0，可能代表关闭)。
  grid_x, grid_y, grid_z: 设置网格单元的尺寸，分别对应 X, Y, Z 方向 (均默认为 0.1)。
  thre_radius: 设置一个名为 thre_radius 的参数，用于指定某个操作的半径 (默认为 0.5)。
  map_resolution: 设置地图的网格分辨率 (默认为 0.05 米/单元格)。
  map_topic_name: 设置地图数据的 ROS 话题名称 (默认为 "map")。
  */
...
// 将点云数据 (cloud) 转换为 ROS 的 nav_msgs::OccupancyGrid 消息 (msg)，用于表示环境中的障碍物信息。
void SetMapTopicMsg(const pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, nav_msgs::OccupancyGrid &msg)
{
  msg.header.seq = 0;
